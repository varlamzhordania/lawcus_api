...
def connect_to_database(username, password, host, port, service_id):
    ...
    try:
        ...
        send_email(
            f"Lawcus Python Sync Failed for {client_name} for database on {get_timestamp()}",
...
def make_api_request(endpoint, params=None, base_url=None, headers=None, **kwargs):
    ...
    try:
        ...
        send_email(
            f"Lawcus Python Sync Failed for {client_name} for {endpoint} on {get_timestamp()}",
...
if __name__ == "__main__":
    ...
    client_id = config['LawcusAPI']['client_id']
    redirect_uri = config['LawcusAPI']['redirect_uri']
    static_auth_code = config['LawcusAPI']['static_auth_code']
    client_secret = config['LawcusAPI']['client_secret']
    state = ''  # Optional
    ...
    if client_id and redirect_uri and client_secret:
        ...
        send_email(
            f"Lawcus Python Sync Failed for {client_name} for API credentials on {get_timestamp()}",
    ...
    if access_token and endpoints:
        for endpoint, params in endpoints:
            if endpoint_config.get(endpoint, 0):
